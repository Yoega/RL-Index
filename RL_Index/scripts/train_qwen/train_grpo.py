...
def parse_args():
    ...
    parser.add_argument("--wandb_project", type=str, default="TongSearch", help="WandB project name")
    parser.add_argument("--wandb_run_name", type=str, default="t1", help="WandB run name")
...
def create_training_args(args) -> GRPOConfig:
    ...
    output_dir = os.path.join(
        args.output_dir,
        f"RL_sbert_{args.training_model_path.split('/')[-1]}_{args.train_data_path.split('/')[-1]}_seed{args.seed}_maxcomp{args.max_completion_length}_numgen{args.num_generations}"
    )
