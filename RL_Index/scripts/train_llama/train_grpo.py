...
from reward import reward_func, init_scoring_model
import wandb
import sys
...
def parse_args():
    ...
    parser.add_argument("--scoring_model_path", type=str, default="sentence-transformers/all-mpnet-base-v2",
                        help="Path to scoring model for reward computation")
    ...
    parser.add_argument("--wandb_project", type=str, default="RL-Index", help="WandB project name")
    parser.add_argument("--wandb_run_name", type=str, default="Llama+SBERT", help="WandB run name")

    # checkpoint path to resume training
    parser.add_argument("--resume_from_checkpoint", default=False, help="Path to checkpoint to resume from")
...
def main():
    ...
    init_scoring_model(args.scoring_model_path)
    ...
    wandb.init(
        ...
        config={
            ...
            "learning_rate": args.learning_rate,
            "beta": args.beta,
            "training_model_path": args.training_model_path,
    ...
    if args.resume_from_checkpoint:
        model, tokenizer = load_training_model_and_tokenizer(args.resume_from_checkpoint)
        print(f"Resumed model from checkpoint: {args.resume_from_checkpoint}")
    else:
